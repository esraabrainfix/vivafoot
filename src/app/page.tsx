...
export default function Home() {
  ...
  return <main>
    ...
    <section className="team-section" id="team"><div className="section-kicker">09 — THE PEOPLE</div><div className="team-heading"><h2>BUILT BY PEOPLE<br /><em>WHO BUILD.</em></h2><p>Engineering, research, and an ambition to make the body&apos;s signals more useful.</p></div><div className="team-grid">{[["ESSRA ELMORSHEDY", "Founder & CEO", "Robotics, AI & brain-computer interface innovator"], ["ABDELALEEM ABDERRAHMAN", "Co-Founder & CTO", "Biomedical & smart systems enthusiast"], ["DR. MOHAMED SALLAM", "Postdoctoral Researcher & Biotech Innovator", "Radiotheranostics & CNS delivery specialist · Postdoc fellow at VIB Ghent"]].map(([name, role, bio], index) => <article className="team-member" key={name}><div className={`portrait portrait-${index + 1}`}><span>{String(index + 1).padStart(2, "0")}</span></div><h3>{name}</h3><strong>{role}</strong><p>{bio}</p></article>)}</div></section>

    <section className="contact-section" id="contact"><div className="section-kicker">10 — CONTACT</div><div className="contact-grid"><div><h2>BUILD THE<br /><em>FUTURE WITH US.</em></h2><p>For research, partnerships, investment, or product inquiries.</p><div className="contact-details"><strong>Esraa Elmorshedy</strong><span>Founder & CEO</span><a href="mailto:esraaelmorshedy2@gmail.com">esraaelmorshedy2@gmail.com</a><a href="tel:+201014743740">+20 10 14743740</a></div></div><form onSubmit={submit} aria-live="polite">{sent ? <div className="form-success"><span>✓</span><h3>Thank you.</h3><p>Your inquiry is ready for the VivaFoot team.</p></div> : <><label htmlFor="contact-name">Name<input id="contact-name" required name="name" /></label><label htmlFor="contact-email">Email<input id="contact-email" required type="email" name="email" /></label><label htmlFor="contact-org">Organization<input id="contact-org" name="organization" /></label><label htmlFor="contact-type">Inquiry type<select id="contact-type" name="type" defaultValue="general"><option value="general">General inquiry</option><option value="research">Research</option><option value="partnership">Partnership</option></select></label><label htmlFor="contact-message">Message<textarea id="contact-message" required name="message" rows={4} /></label>{formError && <p className="form-error" role="alert">{formError}</p>}<button className="button" type="submit" disabled={isSubmitting}>{isSubmitting ? "Sending..." : "Send inquiry"} <span>{isSubmitting ? "" : "↗"}</span></button></>}</form></div></section>
